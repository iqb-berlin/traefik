...
get_release_version() {
  ...
  while read -p 'Please name the desired release tag: ' -er -i "${latest_release}" TARGET_VERSION; do
...
prepare_installation_dir() {
  ...
  while read -p 'Please determine the installation directory: ' -er -i "${PWD}/${APP_NAME}" APP_DIR; do
    if [ ! -e "${APP_DIR}" ]; then
      break
...
download_files() {
  ...
  download_file config/keycloak/import/monitoring-realm.config config/keycloak/import/monitoring-realm.config
  download_file config/keycloak/import/monitoring-realm.json config/keycloak/import/monitoring-realm.json
...
customize_settings() {
  ...
  printf "Default passwords and OAuth2 secrets are generated randomly.\n"
  printf "If you override values manually, use unique secrets per environment.\n\n"
